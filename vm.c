...
#ifndef VM_INSN_STAT
#define VM_INSN_STAT 0
#endif

extern inline rb_cref_t *rb_vm_cref_new_toplevel(void);
extern void vm_cref_dump(const char *mesg, const rb_cref_t *cref);
...
vm_set_top_stack(rb_execution_context_t *ec, const rb_iseq_t *iseq)
{
    ...
    vm_push_frame(ec, iseq, VM_FRAME_MAGIC_TOP | VM_ENV_FLAG_LOCAL | VM_FRAME_FLAG_FINISH, rb_ec_thread_ptr(ec)->top_self,
		  ...
		  iseq->body->rtl_encoded, ec->cfp->sp,
...
invoke_block(rb_execution_context_t *ec, const rb_iseq_t *iseq, VALUE self, const struct rb_captured_block *captured, const rb_cref_t *cref, VALUE type, int opt_pc)
{
    ...
    int except_p = body->catch_except_p;
    ...
    vm_push_frame(ec, iseq, type | VM_FRAME_FLAG_FINISH, self,
		  ...
		  ec->cfp->sp + arg_size,
		  body->local_table_size - arg_size,
		  body->stack_max);
...
/*                  finish
  ...
  struct CONTROL_FRAME {
    ...
    const VALUE *ep, *bp;       // cfp[5], env pointer
    const void *block_code;     // cfp[6], blcok code
  };
...
vm_exec(rb_execution_context_t *ec, int mjit_enable_p)
{
    ...
    _tag.retval = Qnil;
    ec->cfp->ep[VM_ENV_DATA_INDEX_FLAGS] |= VM_FRAME_FLAG_FINISH;
...
#if VM_INSN_STAT
long unsigned all_executed_insns_num, all_executed_insns_len;
#endif
...
rb_execution_context_mark(const rb_execution_context_t *ec)
{
	...
	for(fp = cfp;; fp = RUBY_VM_PREVIOUS_CONTROL_FRAME(fp)) {
	  if ((sp = fp->sp) != NULL) break;
	  if ((iseq = fp->iseq) != NULL && imemo_type((VALUE) iseq) != imemo_iseq) {
	    sp = fp->bp + 1 + iseq->body->temp_vars_num;
	    break;
	  }
	}
	rb_gc_mark_locations(p, sp + 3);
...
#define define_usage_analysis_hash(name) /* shut up rdoc -C */ \
	...
	rb_iseq_t *iseq = rb_iseq_new(0, filename, filename, Qnil, 0, ISEQ_TYPE_TOP);
	...
	th->ec->cfp->iseq = iseq;
	th->ec->cfp->pc = iseq->body->rtl_encoded;
