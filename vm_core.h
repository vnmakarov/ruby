...
struct rb_iseq_constant_body {
    ...
    struct iseq_insn_info insns_info;
    struct iseq_insn_info rtl_insns_info;
...
#define RUBY_VM_THREAD_VM_STACK_SIZE          ( 512 * 1024 * sizeof(VALUE)) /*  512 KB or 1024 KB */
#define RUBY_VM_THREAD_VM_STACK_SIZE_MIN      (   2 * 1024 * sizeof(VALUE)) /*    8 KB or   16 KB */
#define RUBY_VM_THREAD_MACHINE_STACK_SIZE     ( 512 * 1024 * sizeof(VALUE)) /*  512 KB or 1024 KB */
...
typedef struct rb_control_frame_struct {
    ...
    VALUE *ep, *bp;	/* cfp[5] / block[1..2] */
    const void *block_code;     /* cfp[6] / block[3] */ /* iseq or ifunc */
...
VALUE rb_iseq_disasm(const rb_iseq_t *iseq, int iseq_rtl_p);
VALUE rb_iseq_disasm_rtl(const rb_iseq_t *iseq);
int rb_iseq_disasm_insn(VALUE str, const VALUE *iseqval, size_t pos, const rb_iseq_t *iseq, VALUE child, int iseq_rtl_p);
...
enum {
     ...
     *   MMM MMMM MMMM MMMM ____ _FFF FFFF EEEX (LSB)
     *
...
vm_block_iseq(const struct rb_block *block)
{
    switch (vm_block_type(block)) {
      case block_type_iseq: return (rb_iseq_t *)rb_iseq_check(block->as.captured.code.iseq);
