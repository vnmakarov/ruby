...
get_uniq_filename(unsigned long id, const char *prefix, const char *suffix)
{
    ...
    int size = sprint_uniq_filename(buff, sizeof(buff), id, 0, prefix, suffix);
    str = 0;
    ++size;
    str = xmalloc(size);
    if (size <= (int)sizeof(buff)) {
        ...
        sprint_uniq_filename(str, size, id, 0, prefix, suffix);
    }
    return str;
}
...
create_unit(const rb_iseq_t *iseq)
{
    ...
    if (unit == NULL)
        return;
    if ((unit->compile_info = mjit_create_rtl_compile_info(iseq)) == NULL) {
	xfree(unit);
	return;
    }
...
/* TODO: implement */
MJIT_FUNC_EXPORTED void mjit_cancel_all(void) {}

/* Recompile ISEQ if GUARD permits it.  */
void mjit_recompile_iseq(rb_iseq_t *iseq, int (*guard)(struct rb_mjit_compile_info *compile_info)) {
    struct rb_mjit_unit *u = iseq->body->jit_unit;
    
    verbose(2, "Start recompiling iseq in unit %d", u->id);
    CRITICAL_SECTION_START(3, "in recompile iseq");
    if ((ptrdiff_t)iseq->body->jit_func <= (ptrdiff_t)LAST_JIT_ISEQ_FUNC || ! guard(u->compile_info)) {
